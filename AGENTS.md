...
- **Backward self-healing moves require triple proof (FN-5335)**: any backward lifecycle mutation (`in-progress → todo`, `in-review → todo`/`in-progress`, or `in-review → paused+failed`) in `SelfHealingManager` must require all three: (1) dead session (`activeSessionRegistry` clear for `pathsForTask`, no `executingTaskLock`, `isTaskActive !== true`), (2) unusable worktree (`classifyTaskWorktree(...).ok === false`), and (3) no recent activity (stage grace-window staleness + no recent `worktree:incomplete-detected`). If proof fails, the sweep emits `task:<stage>-no-action` and does not mutate lifecycle fields. Per-stage disposition is authoritative in `docs/self-healing-backward-move-audit.md`.
...
- FN-5335 backstop: `packages/engine/src/__tests__/reliability-interactions/backward-move-triple-proof.test.ts` locks triple-proof backward-move gating and no-action audit emission under live-session, usable-worktree, recent-activity, autoMerge-off, liveness-event, and idempotent re-sweep compositions.
